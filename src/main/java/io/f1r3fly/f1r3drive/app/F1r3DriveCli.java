...
class F1r3DriveCli implements Callable<Integer> {
  ...
  @Option(
      ...
      required = true,
      description =
          "Path to the AES cipher key file. A new key is generated if the file does not exist.")
  ...
  @Option(
      names = {"--manual-propose"},
      description =
          "Enable manual block proposing after each deploy and wait for finalization. "
  ...
  public Integer call() throws Exception {
    ...
    F1r3flyBlockchainClient f1R3FlyBlockchainClient =
        new F1r3flyBlockchainClient(
            validatorHost, validatorPort, observerHost, observerPort, manualPropose);
    ...
    try {
      ...
      System.err.println(
          "Signal handling to detect double Ctrl+C not supported: " + t.getMessage());
    }

    // Mount filesystem - unmounting is handled by shutdown hook
    if (wallet != null) {
      f1r3DriveFuse.mountAndUnlockRootDirectory(
          mountPoint, true, fuseDebug, wallet.revAddress, wallet.privateKey, getMountOptions());
    } else {
