...
public class TokenDirectory extends AbstractLocalPath implements Directory {
  ...
  private static final List<Long> denominations =
      Arrays.asList(
  ...
  private long checkBalance() throws F1r3DriveError {
    String checkBalanceRho =
        RholangExpressionConstructor.checkBalanceRho(
            getBlockchainContext().getWalletInfo().revAddress());
    RhoTypes.Expr expr =
        getBlockchainContext().getBlockchainClient().exploratoryDeploy(checkBalanceRho);
   ...
   *     <p>Example: 1000 -> 100, 100, 100, 100, 100, 100, 100, 100, 100, 100 or 14 -> 10, 4 or 10
   *     -> 10 or 9 -> 9 or 99 -> 10, 10, 10, 10, 10, 10, 10, 10, 10, 9
   */
  public void change(TokenFile tokenFile) {
    ...
    while (amount > 0) {
      if (amount >= denomination) {
        ...
        String tokenName =
            denomination + "-REV.changed-" + originalDenomination + "." + i++ + ".token";
        LOGGER.info("Creating token " + tokenName + " with value " + denomination);
        TokenFile newTokenFile =
            new TokenFile(getBlockchainContext(), tokenName, this, denomination);
