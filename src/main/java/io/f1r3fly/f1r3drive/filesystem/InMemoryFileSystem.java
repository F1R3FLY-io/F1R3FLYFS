...
public class InMemoryFileSystem implements FileSystem {
  ...
  @NotNull private final RootDirectory rootDirectory;
  @NotNull private final DeployDispatcher deployDispatcher;
  ...
  private List<String> parseRavAddressesFromGenesisBlock(
    ...
    List<DeployServiceCommon.DeployInfo> deploys =
        f1R3FlyBlockchainClient.getGenesisBlock().getDeploysList();

    DeployServiceCommon.DeployInfo tokenInitializeDeploy =
        deploys.stream()
            .filter((deployInfo1 -> deployInfo1.getTerm().contains("systemVaultInitCh")))
            .findFirst()
            .orElseThrow();
  ...
  private Set<Path> createRavAddressDirectories(DeployDispatcher deployDispatcher)
    ...
    List<String> ravAddresses =
        parseRavAddressesFromGenesisBlock(deployDispatcher.getBlockchainClient());
    ...
    for (String address : ravAddresses) {
      children.add(
          new LockedWalletDirectory(
              new BlockchainContext(new RevWalletInfo(address, null), deployDispatcher),
              rootDirectory));
    }
  ...
  public void unlockRootDirectory(String revAddress, String privateKey)
    ...
    if (lockedRoot instanceof LockedWalletDirectory) {
      try {
        UnlockedWalletDirectory unlockedRoot =
            ((LockedWalletDirectory) lockedRoot).unlock(privateKey, deployDispatcher);
        ...
        if (tokenDirectory != null) {
          stateChangeEventsManager.registerEventProcessor(
              ...
              new StateChangeEventProcessor() {
                ...
                public void processEvent(StateChangeEvents event) {
                  if (event
                      instanceof StateChangeEvents.WalletBalanceChanged walletBalanceChanged) {
                    if (walletBalanceChanged
                        .revAddress()
                        .equals(unlockedRoot.getBlockchainContext().getWalletInfo().revAddress())) {
          ...
          logger.warn(
              "Token directory is null for unlocked root directory: {}",
  ...
  public void terminate() {
    ...
    try {
      ...
      logger.warn(
          "Error waiting for background deployments during termination, continuing with cleanup",
          e);
    }
