...
public class BlockchainFile extends AbstractDeployablePath implements File {
  ...
  public BlockchainFile(
      @NotNull BlockchainContext blockchainContext,
      @NotNull String name,
      @NotNull Directory parent) {
    this(blockchainContext, name, parent, true);
  }

  protected BlockchainFile(
      @NotNull BlockchainContext blockchainContext,
      @NotNull String name,
      @NotNull Directory parent,
      boolean sendToShard) {
  ...
  private void enqueueCreatingFile() {
    refreshLastUpdated();
    String rholang =
        RholangExpressionConstructor.sendEmptyFileIntoNewChanel(
            getAbsolutePath(), getLastUpdated());
    ...
    try {
      ...
      if (!isOtherChunksDeployed) {
        if (!otherChunks.isEmpty()) {
          refreshLastUpdated();
          enqueueMutation(
              RholangExpressionConstructor.updateOtherChunksMap(getAbsolutePath(), otherChunks));
        }
